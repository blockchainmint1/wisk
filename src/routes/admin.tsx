...
function OrderDetail({ publicId }: { publicId: string }) {
  ...
  // Wrap/unwrap are 1:1 (minus fee), so derive the source-token amount
  // from the quote. `source_amount_usd` is a USD figure — never display it
  // labeled as a token.
  const feeMul = 1 - Math.abs(Number(order.premium_bps ?? 0)) / 10_000;
  const quotedInValue = feeMul > 0
    ? Number(order.quoted_dest_out) / feeMul
    : Number(order.quoted_dest_out);
